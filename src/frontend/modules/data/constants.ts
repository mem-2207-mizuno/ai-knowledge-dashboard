...
export const CATEGORY_FORM_CONFIGS: CategoryFormConfig[] = Array.isArray(referenceData.categories)
  ? referenceData.categories
  : [];

export const KNOWN_TAGS: TagRecord[] = Array.isArray(referenceData.tags) ? referenceData.tags : [];
...
export const FORM_CATEGORY_OPTIONS: CategoryFormConfig[] =
  CATEGORY_FORM_CONFIGS.length > 0
    ...
    : CATEGORY_CONFIG.filter(config => config.id && config.id !== 'all').map(config => ({
