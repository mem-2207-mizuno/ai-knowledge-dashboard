...
| タイトル | ナレッジのタイトル | 文字列 |
| URL | 記事のURL | 文字列 |
| コメント | 投稿者による説明・コメント（フリースペース） | 文字列 |
| タグ | カテゴリやタグ（カンマ区切りなど） | 文字列 |
| 投稿日時 | 投稿された日時 | 日時 |
| 投稿者 | 投稿者の名前 | 文字列 |
...
5. **スプレッドシートの準備**
   ...
   - 以下のカラムを1行目に設定：
     ...
     I1: いいね数
     ```
   - `src/Code.ts` の `SPREADSHEET_ID` を実際のスプレッドシートIDに置き換える
...
- `src/Code.ts` の `SPREADSHEET_ID` は必ず実際のスプレッドシートIDに変更してください
- スプレッドシートの1行目はヘッダー行として使用されます
- コメント履歴はJSON形式で保存されます（空の場合は空文字列）
- タグはカンマ区切りで入力してください（例: `TypeScript,GAS,開発`）
